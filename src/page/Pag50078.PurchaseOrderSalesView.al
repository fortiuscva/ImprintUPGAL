page 50078 "Purchase Order-Sales View"
{
    ...
    // IMP1.01,01-Feb-16,ST: Modified "Source Table View". Old Value: WHERE(Document Type=FILTER(Order)),New Value: WHERE(Document Type=FILTER(Order),PO Sample=CONST(No))
    // IMP1.02,09/26/16,ST: Added new factbox "Purch. Line UOM Fact Box" to the page.
    Caption = 'Purchase Order';
    Editable = false;
    PageType = Document;
    RefreshOnActivate = true;
    SourceTable = "Purchase Header";
    SourceTableView = WHERE("Document Type"=FILTER(Order), "PO Sample"=CONST(false));
    ...
    layout
    {
        ...
        area(factboxes)
        {
            ...
            part(Control1100796000; "Purch. Line UOM Fact Box")
            {
                Provider = PurchLines;
