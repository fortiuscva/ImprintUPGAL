report 50002 "SP  Statistics by Inv. Cost"
{
    // <changelog>
    ...
    // </changelog>
    DefaultLayout = RDLC;
    RDLCLayout = './SP  Statistics by Inv. Cost.rdl';
    ApplicationArea = all;
        ...
        dataitem("Salesperson/Purchaser"; "Salesperson/Purchaser")
        {
            ...
            dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
            {
                ...
                begin
                    ...
                    valueentry."Cost Amount (Actual)":=-1 * valueentry."Cost Amount (Actual)";
                    if "Document Type" = "Document Type"::Invoice then begin
                        salesinvline.Reset;
                        salesinvline.SetCurrentKey(Type, "No.");
                        salesinvline.SetRange("Document No.", "Document No.");
                        salesinvline.SetFilter(Type, '<>%1', salesinvline.Type::Item);
                        if salesinvline.FindSet then repeat valueentry."Cost Amount (Actual)"+=salesinvline.Quantity * salesinvline."Unit Cost (LCY)" until salesinvline.Next = 0;
                    end;
                    if "Document Type" = "Document Type"::"Credit Memo" then begin
                        cmline.Reset;
                        cmline.SetCurrentKey(Type, "No.");
                        cmline.SetRange("Document No.", "Document No.");
                        cmline.SetFilter(Type, '<>%1', salesinvline.Type::Item);
                        if cmline.FindSet then repeat valueentry."Cost Amount (Actual)"-=cmline.Quantity * cmline."Unit Cost (LCY)" until cmline.Next = 0;
                    end;
                    "Profit (LCY)":="Sales (LCY)" - valueentry."Cost Amount (Actual)";
    ...
    cmline: Record "Sales Cr.Memo Line";
}
