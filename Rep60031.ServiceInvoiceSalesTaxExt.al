report 60031 "Service Invoice-Sales TaxExt"
{
    // version NAVNA14.03
    DefaultLayout = RDLC;
    RDLCLayout = './Service Invoice-Sales Tax.rdl';
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(ServInvLine; "Integer")
                    {
                        ...
                        begin
                            ...
                            if OnLineNumber = 1 then TempServInvoiceLine.Find('-')
                            else
                                TempServInvoiceLine.Next;
                            OrderedQuantity:=0;
                            if "Service Invoice Header"."Order No." = '' then OrderedQuantity:=TempServInvoiceLine.Quantity
                            else if OrderLine.Get(1, "Service Invoice Header"."Order No.", TempServInvoiceLine."Line No.")then OrderedQuantity:=OrderLine.Quantity
                                else
                                begin
                            ...
                            if TempServInvoiceLine.Amount <> TempServInvoiceLine."Amount Including VAT" then TaxLiable:=TempServInvoiceLine.Amount
                            else
                                TaxLiable:=0;
                            AmountExclInvDisc:=TempServInvoiceLine.Amount + TempServInvoiceLine."Inv. Discount Amount";
                            if TempServInvoiceLine.Quantity = 0 then UnitPriceToPrint:=0 // so it won't print
                            else
 ...
 Clear(CopyTxt)
            ...
            begin
                ...
                if "Tax Area Code" <> '' then begin
                    ...
                    if TaxArea."Use External Tax Engine" then SalesTaxCalc.CallExternalTaxEngineForDoc(DATABASE::"Service Invoice Header", 0, "No.")
                    else
                    begin
