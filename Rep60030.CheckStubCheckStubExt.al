report 60030 "Check (Stub/Check/Stub)Ext"
{
    // version NAVNA14.10
    DefaultLayout = RDLC;
    RDLCLayout = './Check (StubCheckStub).rdl';
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            dataitem(CheckPages; Integer)
            {
                ...
                dataitem(PrintSettledLoop; Integer)
                {
                    ...
                    trigger OnPreDataItem();
                    begin
                        PrintCheckHelper.PrintSettledLoopHelper(CustLedgEntry, VendLedgEntry, GenJnlLine, BalancingType, BalancingNo, FoundLast, TestPrint, FirstPage, FoundNegative, ApplyMethod);
                    ...
                    CASE BalancingType OF
                        ...
                        BalancingType::Customer:
                            BEGIN
                                ...
                                IF Cust."Privacy Blocked" THEN ERROR(PrivacyBlockedErr, Cust.TABLECAPTION, Cust."No.");
                        ...
                        BalancingType::Vendor:
                            BEGIN
                                ...
                                IF Vend."Privacy Blocked" THEN ERROR(PrivacyBlockedErr, Vend.TABLECAPTION, Vend."No.");
                        ...
                        BalancingType::Employee:
                            BEGIN
                                ...
                                IF Employee."Privacy Blocked" THEN ERROR(BlockedEmplForCheckErr, Employee."No.");
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(OneCheckPerVendorPerDocumentNo; OneCheckPrVendor)
                    {
                        ...
                        MultiLine = true;
                        ToolTipML = ENU = 'Specifies if only one check is printed per vendor for each document number.', ESM = 'Especifica si solo se imprime un único cheque por proveedor para cada número de documento.', FRC = 'Indique si un seul chèque est imprimé par fournisseur et par numéro de document.', ENC = 'Specifies if only one cheque is printed per vendor for each document number.';
                    }
                    field(ReprintChecks; ReprintChecks)
                    {
                        ...
                        ToolTipML = ENU = 'Specifies if checks are printed again if you canceled the printing due to a problem.', ESM = 'Especifica si los cheques se imprimen de nuevo después de cancelar la impresión debido a un problema.', FRC = 'Indique si les chèques sont imprimés à nouveau si vous avez annulé l''impression suite à un problème.', ENC = 'Specifies if cheques are printed again if you cancelled the printing due to a problem.';
                    }
                    field(TestPrinting; TestPrint)
                    {
                        ApplicationArea = Basic, Suite;
                        CaptionML = ENU = 'Test Print', ESM = 'Test impresión', FRC = 'Épreuve', ENC = 'Test Print';
                        ToolTipML = ENU = 'Specifies if you want to print the checks on blank paper before you print them on check forms.', ESM = 'Especifica si desea imprimir los cheques con papel en blanco antes de imprimirlos con formularios preimpresos.', FRC = 'Indique si vous souhaitez imprimer les chèques sur papier blanc avant de les imprimer sous forme de chèque.', ENC = 'Specifies if you want to print the cheques on blank paper before you print them on cheque forms.';
                    }
                    field(PreprintedStub; PreprintedStub)
                    {
                        ...
                        ToolTipML = ENU = 'Specifies if you use check forms with preprinted stubs.', ESM = 'Especifica si usa formularios de cheques con series preimpresas.', FRC = 'Indique si vous utilisez les formulaires chèque avec souches pré-imprimées.', ENC = 'Specifies if you use cheque forms with preprinted stubs.';
                    }
    ...
    var
        Text000: TextConst ENU = 'Preview is not allowed.', ESM = 'No está permitida la vista previa.', FRC = 'Impossible d''afficher à l''écran.', ENC = 'Preview is not allowed.';
        ...
        Text014: TextConst ENU = 'Check for %1 %2', ESM = 'Cheque para %1 %2', FRC = 'Chèque pour %1 %2', ENC = 'Cheque for %1 %2';
        ...
        Text030: TextConst ENU = ' is already applied to %1 %2 for customer %3.', ESM = ' ya se aplicó a %1 %2 para el cliente %3.', FRC = ' est déjà affecté à %1 %2 pour le client %3.', ENC = ' is already applied to %1 %2 for customer %3.';
        Text031: TextConst ENU = ' is already applied to %1 %2 for vendor %3.', ESM = ' ya se aplicó a %1 %2 para el proveedor %3.', FRC = ' est déjà affecté à %1 %2 pour le fournisseur %3.', ENC = ' is already applied to %1 %2 for vendor %3.';
        ...
        LineAmount2: Decimal;
        GLSetup: Record 98;
        Text064: TextConst ENU = '%1 must not be %2 for %3 %4.', ESM = '%1 no debe ser %2 para %3 %4.', FRC = '%1 ne doit pas être %2 pour %3 %4.', ENC = '%1 must not be %2 for %3 %4.';
        ...
        USText004: TextConst ENU = 'Last Check No. must include at least one digit, so that it can be incremented.', ESM = 'El campo Òlt. n.º cheque debe incluir al menos un dígito, de modo que pueda incrementarse.', FRC = 'Le n° du dernier chèque doit comprendre au moins un chiffre afin de pouvoir être incrémenté.', ENC = 'Last Cheque No. must include at least one digit, so that it can be incremented.';
        DateIndicator: Text[10];
        CheckDateFormat: Option " ","MM DD YYYY","DD MM YYYY","YYYY MM DD";
        CheckStyle: Option " ",US,CA; //B2BMSOn06Feb2023-Added Empty Option
        CheckLanguage: Integer;
        DateSeparator: Option " ","-",".","/";
        DollarSignBefore: Code[5];
        DollarSignAfter: Code[5];
        PrnChkCompanyAddr: array[3, 8] of Text[50];
        PrnChkCheckToAddr: array[3, 8] of Text[50];
        PrnChkCheckNoText: array[3] of Text[30];
        PrnChkCheckDateText: array[3] of Text[30];
        PrnChkCheckAmountText: array[3] of Text[30];
        PrnChkDescriptionLine: array[3, 2] of Text[80];
        PrnChkVoidText: array[3] of Text[30];
        PrnChkDateIndicator: array[3] of Text[10];
        PrnChkCurrencyCode: array[3] of Code[10];
        USText006: TextConst ENU = 'You cannot use the <blank> %1 option with a Canadian style check. Please check %2 %3.', ESM = 'No puede usar la opción %1 de <blank> con un cheque de estilo canadiense. Compruebe %2 %3.', FRC = 'Vous ne pouvez pas utiliser l''option %1 <vide> avec un chèque canadien. Veuillez vérifier %2 %3', ENC = 'You cannot use the <blank> %1 option with a Canadian style cheque. Please check %2 %3.';
        USText007: TextConst ENU = 'You cannot use the Spanish %1 option with a Canadian style check. Please check %2 %3.', ESM = 'No puede usar la opción %1 de español con un cheque de estilo canadiense. Compruebe %2 %3.', FRC = 'Vous ne pouvez pas utiliser l''option %1 Espagnol avec un chèque canadien. Veuillez vérifier %2 %3', ENC = 'You cannot use the Spanish %1 option with a Canadian style cheque. Please check %2 %3.';
        Stub2LineNo: Integer;
        ...
        NetAmountCaptionLbl: Label 'Net Amount';
