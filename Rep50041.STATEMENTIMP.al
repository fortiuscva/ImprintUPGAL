report 50041 "STATEMENTIMP"
{
    // <changelog>
    //   <add id="NA0000" dev="ELYNCH" date="2004-03-30" area="REPORTS SR" feature="622"
    //     releaseversion="NAVUS3.70">NA Customer Statement</add>
    //   <change id="NA0001" dev="JNOZZI" date="2004-04-07" area="REPORTS SR" feature="622"
    //     baseversion="NAVUS3.70" releaseversion="NAVNA4.00">Add new aging algorithm, plus minor cleanup.</change>
    //   <change id="NA0002" dev="JNOZZI" date="2004-06-07" area="REPORTS SR" feature="622"
    //     baseversion="NAVNA4.00" releaseversion="NAVNA4.00">Due to new &quot;partial payment&quot; functionality, add a
    //     new method of tracing applications.</change>
    //   <change id="NA0003" dev="ELYNCH" date="2005-05-03" area="REPORTS SR"  request="8229"
    //     baseversion="NAVNA4.00" releaseversion="NAVNA4.00.01">Added Captions to Request form option fields</change>
    //   <change id="NA0004" dev="JNOZZI" date="2006-09-15" area="BUG FIX"  request="19290"
    //     baseversion="NAVNA4.00.01" releaseversion="NAVNA4.00.03">Move Code that sets text of Aging Bucket Headings so
    //     that it is executed AFTER the language of the individual customer is set.</change>
    //   <change id="NA0005" dev="JNOZZI" date="2006-06-05" area="REPORTS SR" feature="337"
    //     baseversion="NAVNA4.00.03" releaseversion="NAVNA5.00">Implement the W1 Logo Position functionality.</change>
    //   <change id="NA0006" dev="V-RAWAN" date="2008-06-12" area="REPORTS SR" feature="NC14261"
    //     baseversion="NAVNA5.00" releaseversion="NAVNA6.00">Report Transformation - local Reports Layout.</change>
    //   <change id="NA0007" dev="JNOZZI" date="2008-09-10" area="REPORTS SR" feature="40660"
    //     baseversion="NAVNA6.00" releaseversion="NAVNA6.00">Correct issue where amounts would end up in the wrong
    //     column.</change>
    // </changelog>
    // IMP1.01,03/24/17,SK: Created new report (Save as from R50037.
    //                       Added code in "Cust. Ledger Entry - OnAfterGetRecord()", "AfterStmntDateEntry - OnAfterGetRecord()","CustLedgerEntry4 - OnAfterGetRecord()" and declared global variable.
    // IMP1.02,02/06/18,SK: Added code in "Customer - OnAfterGetRecord()" to change the Aging Captions.
    // IMP1.03,02/20/18,SK: Added Veribage at Page Footer in layout as user required.
    // IMP1.04,03/15/18,SK: Changed the Statement Date as Today when user ran the report.
    //         03/16/18,SK: Changed the buckets range.
    DefaultLayout = RDLC;
    RDLCLayout = './STATEMENTIMP.rdl';
    ApplicationArea = all;
            ...
            begin
                ...
                if AgingMethod <> AgingMethod::None then begin
                    ...
                    if AgingMethod = AgingMethod::"Due Date" then begin
                        ...
                        AgingHead[2] := Text008 + ' ' + Format(PeriodEndingDate[1] - PeriodEndingDate[3]) + Text009;
                    end
                    else begin
                        //FOR i := 2 TO 4 DO
                        ...
                        //IMP1.02 Start
                        AgingHead[1] := Text008 + ' ' + Format(PeriodEndingDate[1] - PeriodEndingDate[2] - 1) + Text009;
                        //IMP1.02 End
                        AgingHead[2] := Format(PeriodEndingDate[1] - PeriodEndingDate[2] + 1) + ' - ' + Format(PeriodEndingDate[1] - PeriodEndingDate[3]) + Text009;
                        //IMP1.02 Start
                        AgingHead[2] := Format(PeriodEndingDate[1] - PeriodEndingDate[2]) + ' - ' + Format(PeriodEndingDate[1] - PeriodEndingDate[3] - 2) + Text009;
                        //IMP1.02 End
                    end;
                    ...
                    //IMP1.02 Start
                    AgingHead[3] := Format(PeriodEndingDate[1] - PeriodEndingDate[3] - 1) + ' - ' + Format(PeriodEndingDate[1] - PeriodEndingDate[4] - 3) + Text009;
                    //IMP1.02 End
                    AgingHead[4] := Text011 + ' ' + Format(PeriodEndingDate[1] - PeriodEndingDate[4]) + Text009;
                    //IMP1.02 Start
                    AgingHead[4] := Text011 + ' ' + Format(PeriodEndingDate[1] - PeriodEndingDate[4] - 3) + Text009;
                    //IMP1.02 End
                end;
                // NA0004.end
