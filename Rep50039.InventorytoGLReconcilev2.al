...
{
    DefaultLayout = RDLC;
    RDLCLayout = './Inventory to GL Reconcile v2.rdl';
    ApplicationArea = all;
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            dataitem("Item Ledger Entry"; "Item Ledger Entry")
            {
                ...
                begin
                    if "Applied Entry to Adjust" then AdjustFlagEntry:=true;
            ...
            dataitem(BufferLoop; "Integer")
            {
                ...
                column(purchnotinv; TempBuffer.Purchrga)
                {
                }
                column(salesnotinv; TempBuffer.salesRGA)
                {
                }
                column(wipint; TempBuffer."WIP interim")
                {
                }
                ...
                trigger OnAfterGetRecord()
                begin
                    if Number = 1 then TempBuffer.Find('-')
                    else
                        TempBuffer.Next;
                end;
                trigger OnPostDataItem()
            ...
            trigger OnAfterGetRecord()
            begin
                if not InvPostingGroup.Get("Inventory Posting Group")then Clear(InvPostingGroup);
    ...
    begin
        ...
        if AsOfDate = 0D then Error(Text000);
        if ShowLocations and not ShowVariants then if not "Item Ledger Entry".SetCurrentKey("Item No.", "Location Code")then Error(Text001, "Item Ledger Entry".TableCaption, "Item Ledger Entry".FieldCaption("Item No."), "Item Ledger Entry".FieldCaption("Location Code"));
        if Item.GetFilter("Date Filter") <> '' then Error(Text002, Item.FieldCaption("Date Filter"), Item.TableCaption);
        CompanyInformation.Get;
        ItemFilter:=Item.GetFilters;
        ShowFootnote:=false;
        GLSetup.Get;
        if GLSetup."Additional Reporting Currency" = '' then ShowACY:=false
        else
        begin
    ...
    begin
        ...
        if ValueEntry.Find('-')then repeat if ItemLedgEntry."Document Type" = ItemLedgEntry."Document Type"::"Purchase Return Shipment" then //AK
 RGAShippednotinvoiced+=ValueEntry."Cost Amount (Expected)"; //AK
                if ItemLedgEntry."Document Type" = ItemLedgEntry."Document Type"::"Sales Return Receipt" then //AK
 salesRGA+=ValueEntry."Cost Amount (Expected)"; //AK
                if ItemLedgEntry."Entry Type" <> ItemLedgEntry."Entry Type"::Output then begin //AK added if statemtn
                    ...
                    wip+=ValueEntry."Cost Amount (Expected)";
                //AK Comment code below
                //ExpectedValue := ExpectedValue + ValueEntry."Cost Amount (Expected)";
                //ExpectedValueACY := ExpectedValueACY + ValueEntry."Cost Amount (Expected) (ACY)";
                //ExpectedPostedToGL := ExpectedPostedToGL + ValueEntry."Expected Cost Posted to G/L";
                //ExpectedPostedToGLACY := ExpectedPostedToGLACY + ValueEntry."Exp. Cost Posted to G/L (ACY)";
                if ValueEntry."Expected Cost" and (ValuedQty = 0)then ValuedQty:=ValueEntry."Valued Quantity";
        ...
        if ValuedQty = 0 then ValuedQty:=InvoicedQty
        else if ValuedQty > 0 then begin
    ...
    begin
        ...
        if ShowVariants or ShowLocations then begin
            if(ItemLedgEntry."Variant Code" <> LastVariantCode)then begin
                ...
                if ShowVariants then begin
                    if(ItemLedgEntry."Variant Code" = '') or not(ItemVariant.Get(ItemLedgEntry."Item No.", ItemLedgEntry."Variant Code"))then VariantLabel:=Text007
                    else
            ...
            if(ItemLedgEntry."Location Code" <> LastLocationCode)then begin
                ...
                if ShowLocations then begin
                    if(ItemLedgEntry."Location Code" = '') or not Location.Get(ItemLedgEntry."Location Code")then LocationLabel:=Text008
                    else
        ...
        if NewRow then UpdateTempBuffer();
        if ShowACY then begin
            ...
            TempBuffer.Purchrga+=Round(RGAShippednotinvoiced, Currency."Amount Rounding Precision");
            TempBuffer.salesRGA+=Round(TempBuffer.salesRGA, Currency."Amount Rounding Precision");
            TempBuffer."WIP interim":=Round(wip, Currency."Amount Rounding Precision");
        end
        else
        begin
        ...
        if ShowVariants then TempBuffer."Variant Code":=LastVariantCode;
        if ShowLocations then TempBuffer."Location Code":=LastLocationCode;
    ...
    begin
        if IsCollecting then begin
            if not TempBuffer.Insert then begin
                ...
                AlreadyInsertedTempBuffer.Purchrga+=TempBuffer.Purchrga;
                AlreadyInsertedTempBuffer.salesRGA+=TempBuffer.salesRGA;
                AlreadyInsertedTempBuffer."WIP interim"+=TempBuffer."WIP interim";
