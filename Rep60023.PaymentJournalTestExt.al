report 60023 "Payment Journal - Test Ext"
{
    // version NAVNA14.03
    DefaultLayout = RDLC;
    RDLCLayout = './Payment Journal - Test.rdl';
        ...
        dataitem("Gen. Journal Batch"; "Gen. Journal Batch")
        {
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                dataitem("Gen. Journal Line"; "Gen. Journal Line")
                {
                    ...
                    dataitem(DimensionLoop; "Integer")
                    {
                        ...
                        begin
                            ...
                            repeat OldDimText:=DimText;
                                if DimText = '' then DimText:=StrSubstNo('%1 - %2', DimSetEntry."Dimension Code", DimSetEntry."Dimension Value Code")
                                else
                    ...
                    begin
                        ...
                        if not EmptyLine then begin
                            ...
                            AmountError:=false;
                            if("Account No." = '') and ("Bal. Account No." = '')then AddError(StrSubstNo(Text001, FieldCaption("Account No."), FieldCaption("Bal. Account No.")))
                            ...
                            CheckICDocument;
                            if "Account No." <> '' then case "Account Type" of "Account Type"::"G/L Account": begin
                                ...
                                "Account Type"::Customer, "Account Type"::Vendor: begin
                                    ...
                                    if "Document Type".AsInteger() <> 0 then begin
                                        if "Account Type" = "Account Type"::Customer then case "Document Type" of "Document Type"::"Credit Memo": WarningIfPositiveAmt("Gen. Journal Line");
                                            "Document Type"::Payment: if("Applies-to Doc. Type" = "Applies-to Doc. Type"::"Credit Memo") and ("Applies-to Doc. No." <> '')then WarningIfNegativeAmt("Gen. Journal Line")
                                                else
                                                    WarningIfPositiveAmt("Gen. Journal Line");
                                            "Document Type"::Refund: WarningIfNegativeAmt("Gen. Journal Line");
                                            else
                                                WarningIfNegativeAmt("Gen. Journal Line");
                                            end
                                        else
                                            case "Document Type" of "Document Type"::"Credit Memo": WarningIfNegativeAmt("Gen. Journal Line");
                                            "Document Type"::Payment: if("Applies-to Doc. Type" = "Applies-to Doc. Type"::"Credit Memo") and ("Applies-to Doc. No." <> '')then WarningIfPositiveAmt("Gen. Journal Line")
                                                else
                                                    WarningIfNegativeAmt("Gen. Journal Line");
                                            "Document Type"::Refund: WarningIfPositiveAmt("Gen. Journal Line");
                                            else
                                                WarningIfPositiveAmt("Gen. Journal Line");
                                            end end;
                                ...
                                "Account Type"::"Fixed Asset": TestFixedAsset("Gen. Journal Line");
                                end;
                            if "Bal. Account No." <> '' then case "Bal. Account Type" of "Bal. Account Type"::"G/L Account": begin
                                ...
                                "Bal. Account Type"::Customer, "Bal. Account Type"::Vendor: begin
                                    ...
                                    if "Document Type" <> 0 then begin
                                        if("Bal. Account Type" = "Bal. Account Type"::Customer) = ("Document Type" in["Document Type"::Payment, "Document Type"::"Credit Memo"])then WarningIfNegativeAmt("Gen. Journal Line")
                                        else
                                ...
                                "Bal. Account Type"::"Fixed Asset": TestFixedAsset("Gen. Journal Line");
                                end;
                            ...
                            if "Posting Date" = 0D then AddError(StrSubstNo(Text002, FieldCaption("Posting Date")))
                            else
                            begin
                            ...
                            if "Document No." = '' then AddError(StrSubstNo(Text002, FieldCaption("Document No.")))
                            ...
                            if(("Account Type" = "Account Type"::"G/L Account") and ("Bal. Account Type" = "Bal. Account Type"::"G/L Account")) or ("Document Type" <> "Document Type"::Invoice)then if PaymentTerms.Get("Payment Terms Code")then begin
                            ...
                            if "Account No." <> '' then case "Account Type" of "Account Type"::"G/L Account": CheckGLAcc("Gen. Journal Line", AccName);
                                "Account Type"::Customer: CheckCust("Gen. Journal Line", AccName);
                                "Account Type"::Vendor: CheckVend("Gen. Journal Line", AccName);
                                "Account Type"::"Bank Account": CheckBankAcc("Gen. Journal Line", AccName);
                                "Account Type"::"Fixed Asset": CheckFixedAsset("Gen. Journal Line", AccName);
                                "Account Type"::"IC Partner": CheckICPartner("Gen. Journal Line", AccName);
                                end;
                            if "Bal. Account No." <> '' then begin
                                ExchAccGLJnlLine.Run("Gen. Journal Line");
                                case "Account Type" of "Account Type"::"G/L Account": CheckGLAcc("Gen. Journal Line", BalAccName);
                                "Account Type"::Customer: CheckCust("Gen. Journal Line", BalAccName);
                                "Account Type"::Vendor: CheckVend("Gen. Journal Line", BalAccName);
                                "Account Type"::"Bank Account": CheckBankAcc("Gen. Journal Line", BalAccName);
                                "Account Type"::"Fixed Asset": CheckFixedAsset("Gen. Journal Line", BalAccName);
                                "Account Type"::"IC Partner": CheckICPartner("Gen. Journal Line", AccName);
                                end;
                                ExchAccGLJnlLine.Run("Gen. Journal Line");
                            end;
                ...
                dataitem(ReconcileLoop; "Integer")
                {
                    ...
                    trigger OnAfterGetRecord();
                    begin
                        if Number = 1 then GLAccNetChange.Find('-')
                        else
                            GLAccNetChange.Next;
                    end;
                    trigger OnPostDataItem();
    ...
    var Text000: TextConst ENU = '%1 cannot be filtered when you post recurring journals.', ESM = 'No se puede filtrar %1 cuando registra diarios periódicos.', FRC = '%1 ne peut être filtré lorsque vous reportez des journaux récurrents.', ENC = '%1 cannot be filtered when you post recurring journals.';
    ...
    begin
        ...
        if(LastDate <> 0D) and (LastDocNo <> '') and ((NextGenJnlLine."Posting Date" <> LastDate) or (NextGenJnlLine."Document Type" <> LastDocType) or (NextGenJnlLine."Document No." <> LastDocNo) or (NextGenJnlLine."Line No." = LastLineNo))then begin
            if GenJnlTemplate."Force Doc. Balance" then begin
                case true of DocBalance <> 0: AddError(StrSubstNo(Text025, SelectStr(LastDocType + 1, Text063), LastDocNo, DocBalance));
                DocBalanceReverse <> 0: AddError(StrSubstNo(Text026, SelectStr(LastDocType + 1, Text063), LastDocNo, DocBalanceReverse));
                end;
                DocBalance:=0;
                DocBalanceReverse:=0;
            end;
    ...
    begin
        ...
        begin
            ...
            if GenJnlLine."Gen. Posting Type" > 0 then begin
                ...
                else if GenJnlLine."VAT Calculation Type" <> VATPostingSetup."VAT Calculation Type" then AddError(StrSubstNo(Text037, GenJnlLine.FieldCaption("VAT Calculation Type"), VATPostingSetup."VAT Calculation Type"))end;
    ...
    begin
        ...
        begin
            ...
            CustPosting:=true;
            TestPostingType;
            if GenJnlLine."Recurring Method" = 0 then if GenJnlLine."Document Type" in[GenJnlLine."Document Type"::Invoice, GenJnlLine."Document Type"::"Credit Memo", GenJnlLine."Document Type"::"Finance Charge Memo", GenJnlLine."Document Type"::Reminder]then begin
    ...
    begin
        ...
        begin
            ...
            if GenJnlLine."Currency Code" <> '' then if not Currency.Get(GenJnlLine."Currency Code")then AddError(StrSubstNo(Text038, GenJnlLine."Currency Code"));
            if(Vend."IC Partner Code" <> '') and (GenJnlTemplate.Type = GenJnlTemplate.Type::Intercompany)then if ICPartner.Get(Cust."IC Partner Code")then begin
            ...
            if(CurrentICPartner <> '') and ("Gen. Journal Line"."IC Direction" = "Gen. Journal Line"."IC Direction"::Outgoing)then begin
                ...
                begin
                    if(("Gen. Journal Line"."Account Type" in["Gen. Journal Line"."Account Type"::"G/L Account", "Gen. Journal Line"."Account Type"::"Bank Account"]) and ("Gen. Journal Line"."Account No." <> '')) xor (("Gen. Journal Line"."Bal. Account Type" in["Gen. Journal Line"."Bal. Account Type"::"G/L Account", "Gen. Journal Line"."Account Type"::"Bank Account"]) and ("Gen. Journal Line"."Bal. Account No." <> ''))then begin
                        if "Gen. Journal Line"."IC Partner G/L Acc. No." = '' then AddError(StrSubstNo(Text002, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No.")))
                        else
                        begin
                            if "IC G/L Account".Get("Gen. Journal Line"."IC Partner G/L Acc. No.")then if "IC G/L Account".Blocked then AddError(StrSubstNo(Text032, "IC G/L Account".FieldCaption(Blocked), false, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No."), "Gen. Journal Line"."IC Partner G/L Acc. No."));
                        end;
                    end
                    else if "Gen. Journal Line"."IC Partner G/L Acc. No." <> '' then AddError(StrSubstNo(Text009, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No.")));
                end;
            end
            else if "Gen. Journal Line"."IC Partner G/L Acc. No." <> '' then begin
                    if "Gen. Journal Line"."IC Direction" = "Gen. Journal Line"."IC Direction"::Incoming then AddError(StrSubstNo(Text069, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No."), "Gen. Journal Line".FieldCaption("IC Direction"), Format("Gen. Journal Line"."IC Direction")));
                    if CurrentICPartner = '' then AddError(StrSubstNo(Text070, "Gen. Journal Line".FieldCaption("IC Partner G/L Acc. No.")));
                end;
        end;
    end;
}
